...
public enum DNSWireFormat {
	...
	public static func decodeTXTRecords(_ data: Data) throws -> [String] {
		...
		for _ in 0..<ancount {
			...
			guard offset + 10 <= bytes.count else { throw DecodeError.truncated }
			let type = beUInt16(bytes, at: offset)
			let rdlength = Int(beUInt16(bytes, at: offset + 8))
			offset += 10
			guard offset + rdlength <= bytes.count else { throw DecodeError.truncated }
			if type == typeTXT {
				results.append(
					joinedCharacterStrings(bytes[offset..<offset + rdlength]))
			}
			offset += rdlength
		}
		return results
	...
	private static func skipName(_ bytes: [UInt8], at start: Int) throws -> Int {
		...
		while true {
			...
			if length == 0 {
				...
				guard offset + 1 < bytes.count else { throw DecodeError.truncated }
